...
Lounch docker image with passing the "all" argument to perform "m all" in docker.
...
"rootfs_arm64.raw.img" is a plain image of debian buster which does not include any goole edge tpu specific files.
The rootfs is formated with falocate and mkfs.ext. And then extract files using "multistrap" and it's configuration file (multistrap.conf).
"qemu-aarch64-static is also copied into the rootfs" to perfrom the installation of aarch64 .deb files on x86 host.
...
pbuilder support the this native compile option, in addition to the cross environement (cross-base.tgz).
...
In the chroot process, aarch64's python is additionaly installed, and the edgetpuvision's python scripts are tested and installed.
Finally the scriptes are packed into the .deb file.
...
The packege specified with "binary" (e.g. GPU [vivante] binary) is not build, but just fetched the pre-build binary blobs from git.
